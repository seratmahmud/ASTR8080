...
if __name__ == '__main__':
    ...
    ax.plot(x,y,'.',color='black',markersize=6,label=r'$x+2x^2+3x^3$')
    ax.plot(x,y2,'.',color='red',markersize=6,label=r'$x+4x^2$')
    ax.set_xlabel(r'x',fontsize=22)
    ax.set_ylabel(r'y',fontsize=22)
    ax.set_xlim([0,100])
    ax.set_ylim([0,np.max(y)])
    ax.legend(loc='lower right',fontsize=22,markerscale=4)
    fig.tight_layout()
